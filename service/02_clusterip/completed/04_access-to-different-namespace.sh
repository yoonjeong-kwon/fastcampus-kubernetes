...
kubectl apply -f service-in-different-namespace.yaml
...
kubectl exec order-69484f8cb8-4bxjv -n snackbar -- curl -s 10.80.13.248

# snackbar 네임스페이스에 order 컨테이너에서 fancy-snackbar 네임스페이스에 있는 delivery 서비스 도메인 이름으로 요청 실행 후 응답 확인 
kubectl exec order-69484f8cb8-4bxjv -n snackbar -- curl -s delivery.fancy-snackbar

# snackbar 네임스페이스에 order 컨테이너에서 환경변수 목록 조회 - fancy-snackbar 네임스페이스의 delivery 서비스 환경변수 확인 
kubectl exec order-69484f8cb8-4bxjv -n snackbar -- env
