# Pod 생성
kubectl apply -f networking-blue-green.yaml
kubectl apply -f networking-red.yaml
...
kubectl exec blue-green-app -c red-app -- printenv POD_IP NAMESPACE NODE_NAME

# blue-app 컨테이너 -> green-app 컨테이너 /tree, /hello 요청 실행
kubectl exec blue-green-app -c blue-app -- curl -s localhost:8081/tree
kubectl exec blue-green-app -c blue-app -- curl -s localhost:8081/hello

# green-app 컨테이너 -> blue-app 컨테이너 /sky, /hello 요청 실행
kubectl exec blue-green-app -c green-app -- curl -s localhost:8080/sky
kubectl exec blue-green-app -c green-app -- curl -s localhost:8080/hello

# blue-app 컨테이너 -> red-app 컨테이너 /rose, /hello 요청 실행
export RED_POD_IP=$(kubectl get pod red-app -o jsonpath="{.status.podIP}")
echo $RED_POD_IP
kubectl exec blue-green-app -c blue-app -- curl -s $RED_POD_IP:8080/rose
kubectl exec blue-green-app -c blue-app -- curl -s $RED_POD_IP:8080/hello

# red-app 컨테이너 -> blue-app 컨테이너 /sky, /hello 요청 실행
export BLUE_POD_IP=$(kubectl get pod blue-green-app -o jsonpath="{.status.podIP}")
echo $BLUE_POD_IP
kubectl exec red-app -- curl -s $BLUE_POD_IP:8080/sky
kubectl exec red-app -- curl -s $BLUE_POD_IP:8080/hello
